...
export default function SeriesView({ group, profile }) {
  ...
  async function savePrediction({ seriesKey, predicted_winner, predicted_games }) {
    await supabase.from('predictions').upsert({
      user_id: profile.id, group_id: group.id,
      series_key: seriesKey, type: 'series',
      predicted_winner, predicted_games,
  ...
  function getSeries(key, round) {
    ...
    return fromApi || { key, teamA: null, teamB: null, status: 'upcoming', winsA: 0, winsB: 0, winner: null }
  }
  ...
  if (loading) return (
    <div className="page text-center" style={{ paddingTop: 60 }}>🏀 Loading series…</div>
  )

  return (
    <div className="page fade-up">
      {ROUNDS.map(({ label, keys, round }) => {
        const seriesList = keys.map(({ key }) => ({ key, series: getSeries(key, round) }))
          ...
          <div key={round} style={{ marginBottom: 28 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {seriesList.map(({ key, series }) => {
                ...
                const correct = series.status === 'finished' && pick?.predicted_winner === series.winner
                const wrong   = series.status === 'finished' && pick?.predicted_winner && pick.predicted_winner !== series.winner
      ...
      {modal && (
        <PredictionModal
          ...
          onSave={pick => savePrediction({ seriesKey: modal.seriesKey, ...pick })}
...
function SeriesCard({ series, pick, pickable, correct, wrong, onPickClick }) {
  ...
  const statusColor = status === 'active' ? '#ff6b6b' : status === 'finished' ? 'var(--green)' : 'var(--text3)'
  const statusLabel = status === 'active' ? '🔴NOW' : status === 'finished' ? '✅ Done' : '⏳ Upcoming'

  return (
    <div className="card" style={{
      borderColor: status === 'active' ? 'rgba(245,184,65,0.25)' : 'var(--border)',
    }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <span style={{ fontSize: 10, fontFamily: 'Barlow Condensed', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: statusColor }}>
          {statusLabel}
        </span>
        {pickable && (
      ...
      {isTBD ? (
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <TeamPill team={teamA} isWinner={winner === teamA.abbr} isPicked={pick?.predicted_winner === teamA.abbr} />
          <div style={{ textAlign: 'center', padding: '0 8px', minWidth: 60 }}>
            {status !== 'upcoming' ? (
              <span className="condensed" style={{ fontSize: 24, fontWeight: 900, color: 'var(--text2)' }}>
                {winsA} – {winsB}
              </span>
            ) : (
              <span style={{ color: 'var(--text3)', fontSize: 12 }}>VS</span>
          ...
          <TeamPill team={teamB} isWinner={winner === teamB.abbr} isPicked={pick?.predicted_winner === teamB.abbr} right />
        </div>
      )}
      ...
      {pick && (
        <div style={{
          ...
          <span style={{
            ...
            color: correct ? 'var(--green)' : wrong ? 'var(--red)' : 'var(--text3)',
          }}>
            {TEAM_BY_ABBR[pick.predicted_winner]?.abbr || pick.predicted_winner} in {pick.predicted_games} 
            {correct ? ' ✓' : wrong ? ' ✗' : ''}
          </span>
        </div>
      )}
      ...
      {!pick && pickable && (
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--border)', fontSize: 12, color: 'var(--purple)' }}>
          👆 👆 No pick yet
        </div>
      )}
    </div>
...
function TeamPill({ team, isWinner, isPicked, right }) {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: right ? 'row-reverse' : 'row', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ textAlign: right ? 'right' : 'left' }}>
        <div className="condensed" style={{
          ...
          color: isWinner ? 'var(--green)' : isPicked ? '#fff' : 'var(--text2)',
        }}>
          {team.abbr}
        </div>
...
function TBDPill({ right }) {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: right ? 'row-reverse' : 'row', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ textAlign: right ? 'right' : 'left' }}>
        <div className="condensed" style={{ fontWeight: 800, fontSize: 15, color: 'var(--text3)' }}>TBD</div>
        <div style={{ fontSize: 10, color: 'var(--text3)' }}>To be determined</div>
      </div>
    </div>
  )
}
