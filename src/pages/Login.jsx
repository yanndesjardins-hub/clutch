...
export default function Login() {
  ...
  function signupValid() {
    return firstName.trim() && postcode.trim() && email.trim() && pwd.length >= 6
      && playsBasket && followsNBA && followsEuro && emailConsent
  }
  ...
  async function submit(e) {
    ...
    try {
      if (mode === 'signup') {
        ...
        if (pending) { sessionStorage.removeItem('hc_pending_invite'); window.location.href = `/join/${pending}` }
      }
    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
      <div style={{ width:'100%', maxWidth:400 }}>
        ...
        <div className="text-center" style={{ marginBottom:32 }}>
          <img src="/clutch_logo.png" alt="Clutch" style={{ height:120, marginBottom:12 }} />
          <p style={{ color:'var(--text3)', fontSize:16, letterSpacing:2 }}>NBA PLAYOFFS 2026</p>
        </div>
          ...
          {forgotMode ? (
            <form onSubmit={handleForgot} style={{ display:'flex', flexDirection:'column', gap:14 }}>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
                <button type="button" onClick={() => { setForgotMode(false); setErr(''); setMsg('') }}
                  style={{ background:'none', border:'none', color:'var(--text3)', cursor:'pointer', fontSize:18, padding:0 }}>
                  ←
                </button>
                <h3 className="condensed" style={{ fontSize:18, fontWeight:800, letterSpacing:2 }}>
                  Reset Password
                </h3>
              </div>
              ...
              {msg && <p style={{ color:'var(--green)', fontSize:13 }}>{msg}</p>}
              <button className="btn btn-purple btn-full" type="submit" disabled={busy || !email}>
              ...
              <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:14 }}>
                {mode === 'signup' && (<>
                  ...
                  <div>
                    ...
                    <input className="input" type="text" placeholder="e.g. 75011"
                      value={postcode} onChange={e => setPostcode(e.target.value)} required maxLength={10} />
                  </div>
                  <div>
                    <label className="label-form">Do/did you play basketball? *</label>
                  ...
                  <div>
                    <label className="label-form">Do you follow the NBA? *</label>
                    <div style={{ display:'flex', gap:16, marginTop:4 }}>
                  ...
                  <div>
                    <label className="label-form">Do you follow European basketball? *</label>
                    <div style={{ display:'flex', gap:16, marginTop:4 }}>
                  ...
                  <div>
                    <label className="label-form">Your favorite team? (optional)</label>
                    <input className="input" type="text" placeholder="e.g. Miami Heat"
                      value={favTeam} onChange={e => setFavTeam(e.target.value)} />
                  </div>
                </>)}
                ...
                {mode === 'signup' && (
                  <div className="checkbox-row">
                    <input type="checkbox" id="consent" checked={emailConsent}
                      onChange={e => setEmailConsent(e.target.checked)} required />
                    <label htmlFor="consent">
                      I agree to receive email communications from the team that created this game regarding
                      basketball (follow up on this game, other news, events). I can unsubscribe at any time
                ...
                {mode === 'signup' && (
                  <p style={{ fontSize:11, color:'var(--text3)', textAlign:'center' }}>
                    ...
                    <a href="/terms" style={{ color:'var(--purple)' }}>Terms & Conditions</a>
                  </p>
                )}
                ...
                <button className="btn btn-purple btn-full" type="submit"
                  disabled={busy || (mode==='signup' && !signupValid())} style={{ marginTop:4 }}>
                  {busy ? '…' : mode==='signup' ? 'Create my account' : 'Sign in'}
                </button>

                {/* Forgot password */}
                {mode === 'login' && (
                  <button type="button" onClick={() => { setForgotMode(true); setErr(''); setMsg('') }}
